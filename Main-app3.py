...
class EnhancedTextEmotionAnalyzer:
    ...
    def extract_key_phrases_enhanced(self, text, num_phrases=5):
        ...
        for emotion, patterns in self.emotion_patterns.items():
         for word in patterns['words']:
           if word in text_lower_local and len(word) > 2:
            phrases.add(word)
...
def show_text_analysis_enhanced():
    ...
    with col2:
        ...
        st.markdown("### 📊 Text States Preview")
        if text_input:
...
def show_history():
    ...
    for i, entry in enumerate(reversed(filtered_history)):
        ...
        if entry_type == 'text_analysis':
            ...
            with st.expander(f"📝 Text Analysis {len(filtered_history)-i}: {entry.get('timestamp', 'Unknown Date')}"):
                col1, col2 = st.columns([3, 1])
                
                with col1:
                    st.markdown(f"**Text Preview:** {entry.get('text_preview', 'N/A')}")
                    st.markdown(f"**Primary Emotion:** <span style='color:{color}'>{entry.get('dominant_emotion', 'unknown').upper()}</span>", unsafe_allow_html=True)
                    
                    # Safe access to sentiment to avoid KeyError
                    sentiment_value = "N/A"
                    if 'sentiment' in entry:
                        if isinstance(entry['sentiment'], dict):
                            if 'combined' in entry['sentiment'] and entry['sentiment']['combined']:
                                sentiment_value = f"{entry['sentiment']['combined'].get('compound', 0):.2f}"
                            elif 'compound' in entry['sentiment']:
                                sentiment_value = f"{entry['sentiment'].get('compound', 0):.2f}"
                    st.markdown(f"**Sentiment Score:** {sentiment_value}")
                    
                    # Safe access to statistics
                    word_count = "N/A"
                    if 'statistics' in entry and isinstance(entry['statistics'], dict):
                        word_count = entry['statistics'].get('word_count', 'N/A')
                        ...
                        try:
                            ...
                            if st.session_state.username in users:
                                users[st.session_state.username]['history'] = [
                                    ...
                                    and h.get('timestamp') != entry.get('timestamp')
                                ]
                                with open(USER_DB_FILE, 'w') as f:
                                    json.dump(users, f, indent=4)
                        except:
            ...
            with st.expander(f"🎤 Voice Analysis {len(filtered_history)-i}: {entry.get('timestamp', 'Unknown Date')}"):
                col1, col2 = st.columns([3, 1])
                
                with col1:
                    st.markdown(f"**Emotion:** <span style='color:{color}'>{entry.get('dominant_emotion', 'unknown').upper()}</span>", unsafe_allow_html=True)
            ...
            with st.expander(f"😊 Facial Analysis {len(filtered_history)-i}: {entry.get('timestamp', 'Unknown Date')}"):
                col1, col2 = st.columns([3, 1])
